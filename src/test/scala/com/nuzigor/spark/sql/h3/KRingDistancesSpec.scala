...
class KRingDistancesSpec extends H3Spec {
  it should "create rings around h3 index" in {
    ...
    val spatialDf = sparkSession.sql(s"SELECT $functionName(${h3}l, 1)")
    val ring = spatialDf.first().getAs[Seq[Seq[Long]]](0)
    assert(ring.size === 2)
  ...
  it should "return null for null h3" in {
    val spatialDf = sparkSession.sql(s"SELECT $functionName(null, 2)")
    assert(spatialDf.first().isNullAt(0))
  }

  it should "return null for null resolution" in {
    val h3 = 622485130170302463L
    val spatialDf = sparkSession.sql(s"SELECT $functionName(${h3}l, null)")
    assert(spatialDf.first().isNullAt(0))
  }

  it should "not return null for invalid h3" in {
    val spatialDf = sparkSession.sql(s"SELECT $functionName(0l, 2)")
    assert(!spatialDf.first().isNullAt(0))
  }
